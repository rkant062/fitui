...
export const groupByDay = (data) => {
  ...
  // Sort by date keys
  const sortedLabels = Object.keys(days).sort((a, b) => new Date(a) - new Date(b));
...
export const groupByWeek = (data) => {
  ...
  const sortedLabels = Object.keys(weeks).sort((a, b) => {
    ...
    return yearA - yearB || monthOrder.indexOf(monthA) - monthOrder.indexOf(monthB) || parseInt(weekA.slice(1)) - parseInt(weekB.slice(1));
  });
